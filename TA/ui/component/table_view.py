from PySide6.QtWidgets import (
    ...
    QPushButton, QGridLayout, QSizePolicy, QCheckBox
)
...
class TableWidget(QWidget):
    ...
    def init_ui(self):
        ...
        title_row.setColumnStretch(1, 3)
        title_row.setColumnStretch(2, 2)
        title_row.setColumnStretch(3, 2)
        title_row.setColumnStretch(4, 2)
        title_row.setColumnStretch(5, 2)
        title_row.setColumnStretch(6, 2)
        title_row.setColumnStretch(7, 2)

        # Ganti "Select All" dengan label kosong/checkbox kosong (dihilangkan)
    ...
    def populate_cards(self):
        ...
        for card in self.cards:
            card_id = str(card.id)
            self.card_data_map[str(card_id)] = card

            if card_id not in self.card_selection_map:
                self.card_selection_map[str(card_id)] = False
    ...
    def add_card(self, card, card_id):
        ...
        checkbox = QCheckBox()
        checkbox.setChecked(self.card_selection_map.get(card_id, False))
        ...
        checkbox.stateChanged.connect(lambda state, cid=str(card_id): self.checkbox_state_changed(cid, state))
        self.checkbox_map[str(card_id)] = checkbox
        ...
        delete_btn.setStyleSheet("border: 1px solid black; padding: 5px 10px;")
        info_btn = QPushButton("Info")
        info_btn.setStyleSheet("border: 1px solid black; padding: 5px 10px;")
    ...
    def checkbox_state_changed(self, card_id, checked):
        ...
        print(f"Checkbox changed: {card_id} = {checked}")
